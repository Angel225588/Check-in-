...
const PhotoCapture = forwardRef<PhotoCaptureHandle, PhotoCaptureProps>(
  function PhotoCapture({ onProcessed, onTypedResult, apiEndpoint = "/api/ocr", maxFiles = MAX_FILES_DEFAULT }, ref) {
    ...
    const processWithAi = async (file: File, retries = 2): Promise<{ clients: Client[]; docType: "clients" | "vip" | "unknown" } | null> => {
    ...
    const processFile = async (file: File, index: number) => {
      try {
        ...
        if (forceLocal) {
          ...
          let aiResult: { clients: Client[]; docType: "clients" | "vip" | "unknown" } | null = null;
          let aiFailed = false;
          try {
