...
/** The last few people through the door — the "did I already do 224?" check. */
export function RecentsPane({ recents }: { recents: RecentEntry[] }) {
  return (
    <div className={shell} data-role="pane-recents">
      <span className={cap} style={{ color: "var(--tab-idle)" }}>Récents</span>
      <div className="flex-1 min-h-0 flex flex-col justify-center gap-1.5 mt-2">
        {recents.length === 0 && (
          <span className="text-[13px] font-semibold" style={{ color: "var(--tab-idle)" }}>
        ...
        {recents.slice(0, 3).map((r, i) => (
          <div key={`${r.roomNumber}-${i}`} className="flex items-baseline gap-3">
      ...
      <div className="flex-1 min-h-0 overflow-y-auto flex flex-col gap-1.5 mt-2">
        ...
        {notes.map((n) => {
          ...
          return (
            <div
              key={n.id}
              className="rounded-[11px] px-2.5 py-2 flex items-start gap-2 relative overflow-hidden"
      ...
      <div className="flex-1 min-h-0 overflow-y-auto flex flex-col gap-1.5 mt-2">
        {today.length > 0 && (
          <>
            ...
            {today.map((t, i) => (
              <div key={`t${i}`} className="flex items-baseline gap-2.5 text-[13px] font-bold">
        ...
        {stays.slice(0, 4).map((s, i) => (
          <div key={`s${i}`} className="flex items-baseline gap-2.5 text-[13px]">
